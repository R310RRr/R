...
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/SadOz8/Uiiiiiii/refs/heads/main/iiiiiiii"))()

local player = game.Players.LocalPlayer
local displayName = player.DisplayName or player.Name

local window = library:AddWindow("Kill Anyone | - Welcome " .. displayName, {
    main_color = Color3.fromRGB(54, 6, 6),
    min_size = Vector2.new(500, 965),
    can_resize = false,
})
...
local PlayerDrop = Stats:AddDropdown("Select Player", function(Value)
    SelectPlayerName = Value:match("| (.+)") or Value
end)

local function getPlayerDropdownText(player)
    local cleanDisplayName = player.DisplayName:gsub("%s+", " "):gsub("^%s+", ""):gsub("%s+$", "")
    if cleanDisplayName == "" or cleanDisplayName == player.Name then
        return player.Name
    else
        return cleanDisplayName .. " | " .. player.Name
    end
end

for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
    if player ~= LocalPlayer then
        PlayerDrop:Add(getPlayerDropdownText(player))
    end
end
...
game:GetService("Players").PlayerRemoving:Connect(function(player)
    local displayText = getPlayerDropdownText(player)
    PlayerDrop:Remove(displayText)
    
    if SelectPlayerName == player.Name then
        SelectPlayerName = ""
    end
end)
...
local function FormatAbbreviated(number)
    ...
    if number % 1 == 0 then
        return string.format("%.0f", number) .. abbreviations[abbreviationIndex]
    else
        return string.format("%.2f", number):gsub("%.?0+$", "") .. abbreviations[abbreviationIndex]
    end
end
...
local function FormatDisplay(value)
    local normal = ""
    if value % 1 == 0 then
        normal = FormatNumberWithCommas(math.floor(value))
    else
        normal = string.format("%.2f", value):gsub("%.?0+$", "")
    end
    local abbreviated = FormatAbbreviated(value)
    return " " .. normal .. " [ " .. abbreviated .. " ]"
end

local Update = Stats:AddLabel("Strength:")
local Update1 = Stats:AddLabel("Gems:")
local Update2 = Stats:AddLabel("Rebirth:")
local Update3 = Stats:AddLabel("Agility:")
local Update4 = Stats:AddLabel("Durability:")
local Update5 = Stats:AddLabel("Kills:")
local Update6 = Stats:AddLabel("Muscle King Time:")
local Update9 = Stats:AddLabel("Current Map:")
local Update10 = Stats:AddLabel("Custom Size:")
local Update11 = Stats:AddLabel("Custom Speed:")
local Update12 = Stats:AddLabel("Evil Karma:")
local Update13 = Stats:AddLabel("Good Karma:")
    ...
    while task.wait(0.1) do
        if SelectPlayerName ~= "" then
            ...
            if player then
                ...
                Update.Text = "Strength:"
                Update1.Text = "Gems:"
                Update2.Text = "Rebirth:"
                Update3.Text = "Agility:"
                Update4.Text = "Durability:"
                Update5.Text = "Kills:"
                Update6.Text = "Muscle King Time:"
                Update9.Text = "Current Map:"
                Update10.Text = "Custom Size:"
                Update11.Text = "Custom Speed:"
                Update12.Text = "Evil Karma:"
                Update13.Text = "Good Karma:"
            end
        else
            Update.Text = "Strength:"
            Update1.Text = "Gems:"
...
local nothinglabel = Stats:AddLabel("")

Stats:AddLabel("Advanced Stats:").TextSize = 20

local enemyHealthLabel = Stats:AddLabel("Enemy Health: N/A")
enemyHealthLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local playerDamageLabel = Stats:AddLabel("Your Damage: N/A")
playerDamageLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local hitsToKillLabel = Stats:AddLabel("Hits to Kill: N/A")
...
local function calculateHitsToKill(health, damage)
    if damage <= 0 then return "ÃƒÂ¢Ã‹â€ Ã…Â¾" end
...
local function updateAdvancedStats(targetPlayer)
    if not targetPlayer then
        ...
        playerDamageLabel.Text = "Your Damage:"
...
local Killer = window:AddTab("Destroy Anyone")
...
local whitelistDropdown = Killer:AddDropdown("Whitelist Player", function(displayName)

    local playerName
    if displayName:find(" | ") then
        playerName = displayName:match(" | (.+)")
    else
    ...
    if target and target ~= LocalPlayer then
        ...
        whitelistDropdownItems[target.Name] = nil
        rebuildWhitelistDropdown()
    end
end)

local function rebuildWhitelistDropdown()
    for playerName, displayText in pairs(whitelistDropdownItems) do
        whitelistDropdown:Remove(displayText)
    end
    whitelistDropdownItems = {}
    
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and not playerWhitelist[player.Name] then
            local displayText = getPlayerDropdownText(player)
            whitelistDropdown:Add(displayText)
            whitelistDropdownItems[player.Name] = displayText
        end
    end
end
...
local titleLabel = Killer:AddLabel("_______________________________________")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
...
Killer:AddSwitch("Auto Equip Punch", function(state)
    functionsEnabled.autoEquip = state
    task.spawn(function()
        while functionsEnabled.autoEquip do
...
Killer:AddSwitch("Auto Punch [Without Animation]", function(state)
    functionsEnabled.noAnim = state
    task.spawn(function()
        while functionsEnabled.noAnim and muscleEvent do
            pcall(function()
...
Killer:AddSwitch("Auto Punch", function(state)
    functionsEnabled.auto = state
    task.spawn(function()
        while functionsEnabled.auto do
            local punch = getPunch()
            if punch then
...
local megaSwitch = Killer:AddSwitch("NaN Size + Egg (UnderWater)", function(state)
...
local farmPosition = CFrame.new(-12510.5, -1.6537845134735107, 28019.103515625)
...
local megaSwitch = Killer:AddSwitch("NaN Size + Egg (Safe Spot)", function(state)
    megaSwitchEnabled = state
    autoDrinkEnabled = state
...
local autoPetSystem = Killer:AddSwitch("Auto Replace Pets", function(state)
    _G.autoPetSystem = state
    
    if state then
        if not _G.AutoPetGUI then
            local Players = game:GetService("Players")
            local LocalPlayer = Players.LocalPlayer
            ...
            Shadow.Position = UDim2.fromScale(-0.025, -0.05)
            ...
            TopBar.BackgroundColor3 = Color3.fromRGB(30, 136, 229)
            ...
            PetIcon.Size = UDim2.fromOffset(20, 20)
            PetIcon.Position = UDim2.new(0, 8, 0, 5)
            PetIcon.BackgroundTransparency = 1
            PetIcon.Image = "rbxassetid://6023426926"
            PetIcon.Parent = TopBar
            
            local TitleText = Instance.new("TextLabel")
            TitleText.Size = UDim2.new(1, -36, 0, 30)
            TitleText.Position = UDim2.new(0, 30, 0, 0)
            TitleText.BackgroundTransparency = 1
            TitleText.Text = "Auto Replace Pets"
            ...
            StatusContainer.BackgroundColor3 = Color3.fromRGB(30, 30, 35)
            StatusContainer.BackgroundTransparency = 0.3
            StatusContainer.BorderSizePixel = 0
            StatusContainer.Parent = StatusFrame
            
            local ContainerCorner = Instance.new("UICorner")
            ContainerCorner.CornerRadius = UDim.new(0, 6)
            ContainerCorner.Parent = StatusContainer
            
            local StatusIcon = Instance.new("ImageLabel")
            StatusIcon.Size = UDim2.fromOffset(16, 16)
            StatusIcon.Position = UDim2.new(0, 6, 0.5, -8)
            StatusIcon.BackgroundTransparency = 1
            StatusIcon.Image = "rbxassetid://6031094665"
            StatusIcon.ImageColor3 = Color3.fromRGB(76, 175, 80)
            ...
            StatusText.Text = "Sistema Ativo | Pet: Wild Wizard"
            ...
            ToggleButton.Size = UDim2.fromOffset(220, 32)
            ...
            ButtonText.Text = "DESLIGAR SISTEMA"
            ...
            ButtonIcon.Position = UDim2.new(1, -26, 0.5, -8)
            ButtonIcon.BackgroundTransparency = 1
            ButtonIcon.Image = "rbxassetid://6035047409"
            ButtonIcon.ImageColor3 = Color3.fromRGB(255, 100, 100)
            ButtonIcon.Parent = ToggleButton
            
            _G.AutoPetGUI = {
                ScreenGui = FloatingGUI,
            ...
            local function animateButton(button, hover)
                ...
                local goals = {BackgroundColor3 = hover and Color3.fromRGB(70, 70, 85) or Color3.fromRGB(50, 50, 60)}
                TweenService:Create(button, tweenInfo, goals):Play()
            end
            
            ToggleButton.MouseEnter:Connect(function() animateButton(ToggleButton, true) end)
            ToggleButton.MouseLeave:Connect(function() animateButton(ToggleButton, false) end)
            
            local function UnequipAllPets()
                if not systemEnabled or isEquipping then return end
                isEquipping = true
            ...
            local function EquipPetsByName(petName)
                if not systemEnabled or isEquipping then return 0 end
                isEquipping = true
                local count = 0
                for _, folder in ipairs(PetsFolder:GetChildren()) do
                    if folder:IsA("Folder") then
            ...
            local function SetupCharacterMonitoring(character)
                ...
                humanoid.Died:Connect(function()
                    if systemEnabled then
                        StatusText.Text = "Sistema Ativo | Pet: Mighty Monster"
                        UnequipAllPets()
                        task.wait(respawnCooldown)
                        if systemEnabled then
                ...
                character.AncestryChanged:Connect(function(_, parent)
                    if parent == workspace and systemEnabled then
                        task.wait(0.5)
                        if systemEnabled then
                            StatusText.Text = "Sistema Ativo | Pet: Wild Wizard"
                            UnequipAllPets()
            ...
            ToggleButton.MouseButton1Click:Connect(function()
                ...
                if systemEnabled then
                    ...
                    StatusIcon.ImageColor3 = Color3.fromRGB(255, 80, 80)
                    ButtonText.Text = "LIGAR SISTEMA"
                    ButtonIcon.ImageColor3 = Color3.fromRGB(100, 255, 100)
                    UnequipAllPets()
                end
            end)
            ...
            local function update(input)
                ...
                TweenService:Create(StatusFrame, TweenInfo.new(0.05), {Position = newPos}):Play()
            end
            ...
            _G.AutoPetGUI.ScreenGui.Enabled = true
        end
    else
        if _G.AutoPetGUI and _G.AutoPetGUI.ScreenGui then
            _G.AutoPetGUI.ScreenGui.Enabled = false
        end
        _G.autoPetSystem = nil
    end
end)

local BlockEggs = Killer:AddSwitch("Block Protein Egg", function(Value)
    if Value then
        local function disableEgg()
            local backpack = LocalPlayer:FindFirstChild("Backpack")
            if backpack then
        ...
        local conn = game:GetService("RunService").Heartbeat:Connect(disableEgg)
        ...
        mt.__namecall = newcclosure(function(self, ...)
            ...
            if method == "FireServer" then
                ...
                if remoteName == "EatEvent" or remoteName == "ConsumeEvent" or remoteName:find("Eat") then
                    local char = LocalPlayer.Character
                    if char then
                        local tool = char:FindFirstChildWhichIsA("Tool")
                        if tool and tool.Name == "Protein Egg" then
                            return
                        end
                    end
                end
        ...
        if _G.EggConn then
            _G.EggConn:Disconnect()
            _G.EggConn = nil
        end
        
        local mt = getrawmetatable(game)
        setreadonly(mt, false)
        mt.__namecall = _G.EggOldHook
        setreadonly(mt, true)
    end
end)
...
local titleLabel = Killer:AddLabel("Sizes")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
...
local titleLabel = Killer:AddLabel("Whitelisting & Killing")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local nothingLabel = Killer:AddLabel("")

local whitelistLabel = Killer:AddLabel("Whitelist: None")
whitelistLabel.TextSize = 23
whitelistLabel.TextColor3 = Color3.fromRGB(0, 255, 255)
...
local function updateWhitelistLabel()
    ...
    if #names == 0 then
        ...
        whitelistLabel.Text = "Whitelist: " .. text
    end
end
...
local targetLabel = Killer:AddLabel("Blacklist: None")
...
local titleLabel = tabteleport:AddLabel("Settings")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local switch = tabteleport:AddSwitch("Anti-Knockback", function(Value)
    if Value then
...
tabteleport:AddSwitch("Walk On Water", function(bool)
    if bool then
        createParts()
    else
...
tabteleport:AddButton("Remove Ad", function()
    for _, portal in pairs(game:GetDescendants()) do
        if portal.Name == "RobloxForwardPortals" then
            portal:Destroy()
        end
...
tabteleport:AddButton("Shiftlock [Old]", function()
pcall(function()
loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Shiftlock-for-mobile-93971"))()
end)
end)

tabteleport:AddButton("Shiftlock [New]", function()
pcall(function()
loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Shiftlockformobile-105913"))()
end)
end)

local timeDropdown = tabteleport:AddDropdown("Change Time", function(selection)
...
local titleLabel = tabteleport:AddLabel("_______________________________________")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local titleLabel = tabteleport:AddLabel("Teleport")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
...
local titleLabel = tabteleport:AddLabel("Island To Fight V2")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
...
local titleLabel = tabteleport:AddLabel("Optimization")
titleLabel.TextSize = 20
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

tabteleport:AddButton("Remove Textures", function()
...
tabteleport:AddButton("Reduce Graphics", function()
    settings().Rendering.QualityLevel = 1
end)
 
tabteleport:AddButton("Disable Effects", function()
    for _, v in pairs(game:GetDescendants()) do
...
tabteleport:AddButton("Simplify Materials", function()
    for _, v in pairs(game:GetDescendants()) do
...
tabteleport:AddButton("Remove Fog", function()
    local lighting = game:GetService("Lighting")
    lighting.FogEnd = 9e9
end)
 
tabteleport:AddButton("Full Optimization", function()
    for _, v in pairs(game:GetDescendants()) do
        if v:IsA("ParticleEmitter") or v:IsA("Smoke") or v:IsA("Fire") or v:IsA("Sparkles") then
            v.Enabled = false
        end
...
local titleLabel = tabteleport:AddLabel("Rejoin Options")
titleLabel.TextSize = 20
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

tabteleport:AddButton("Rejoin Server", function()
    local ts = game:GetService("TeleportService")
    local p = game:GetService("Players").LocalPlayer
    ts:Teleport(game.PlaceId, p)
end)

local tabgodmode = window:AddTab("God Mode")

local titleLabel = tabgodmode:AddLabel("God Mode Options")
titleLabel.TextSize = 20
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local godModeToggle = false
tabgodmode:AddSwitch("God Mode (V1)", function(State)
    godModeToggle = State
    if State then
        task.spawn(function()
...
tabgodmode:AddSwitch("God Mode (V2)", function(state)
    godModeToggle = state
    if state then
...
local followDropdown = tabgodmode:AddDropdown("Teleport Player (Following)", function(selectedDisplayName)
...
tabgodmode:AddButton("Stop Following", function()
    following = false
    followTarget = nil
end)
...
tabgodmode:AddSwitch("Auto Slam", function(state)
...
local titleLabel = tabgodmode:AddLabel("_______________________________________")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local titleLabel = tabgodmode:AddLabel("Sizes")
titleLabel.TextSize = 20
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

tabgodmode:AddButton("Size NaN", function()
    local sizeToSet = 0/0
    pcall(function()
        game:GetService("ReplicatedStorage").rEvents.changeSpeedSizeRemote:InvokeServer("changeSize", sizeToSet)
    end)
...
tabgodmode:AddButton("Size 2", function()
    pcall(function()
        game:GetService("ReplicatedStorage").rEvents.changeSpeedSizeRemote:InvokeServer("changeSize", 2)
    end)
end)

tabgodmode:AddButton("Size 30", function()
    pcall(function()
        game:GetService("ReplicatedStorage").rEvents.changeSpeedSizeRemote:InvokeServer("changeSize", 30)
    end)
end)
...
local titleLabel = tabgodmode:AddLabel("Essentials")
titleLabel.TextSize = 20
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
...
local switch = tabgodmode:AddSwitch("Walk On Water", function(bool)
    if bool then
        createParts()
    else
        makePartsWalkthrough()
...
tabgodmode:AddSwitch("Auto Clear Inventory", function(state)
    autoEatBoostsEnabled = state
end)

tabgodmode:AddButton("Remove Ad", function()
    for _, portal in pairs(game:GetDescendants()) do
        if portal.Name == "RobloxForwardPortals" then
            portal:Destroy()
        end
...
tabgodmode:AddButton("Shiftlock (New)", function()
pcall(function()
loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Shiftlockformobile-105913"))()
end)
end)

local timeDropdown = tabgodmode:AddDropdown("Change Time", function(selection)
    local lighting = game:GetService("Lighting")
    lighting.Brightness = 2
    lighting.FogEnd = 100000
    lighting.Ambient = Color3.fromRGB(127,127,127)
...
local titleLabel = tabgodmode:AddLabel("Tools")
titleLabel.TextSize = 20
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
...
tabgodmode:AddSwitch("Inf Jump", function(state)
    infJumpEnabled = state
end)

local switch = tabgodmode:AddSwitch("Disable Trade", function(State)
            if State then
...
local showpetsswitch = tabgodmode:AddSwitch("Show Pets", function(bool)
...
local showotherpetsswitch = tabgodmode:AddSwitch("Show Other Pets", function(bool)
    local player = game:GetService("Players").LocalPlayer
...
tabgodmode:AddSwitch("Chat Spy", function(state)
    ...
    if chatSpyEnabled then
        ...
        local function processMessage(speaker, message)
            ...
            local conn = onMessageDone.OnClientEvent:Connect(function(msgData, channel)
                if msgData.SpeakerUserId == speaker.UserId then
                    isPublic = true
                end
            end)
            ...
            if not isPublic then
                game:GetService("StarterGui"):SetCore("ChatMakeSystemMessage", {
                    Text = "[SPY] " .. speaker.Name .. ": " .. cleanMsg,
...
tabgodmode:AddButton("Korblox", function()
    ...
    if char then
        ...
        if rLower and rUpper and rFoot then
            ...
            rFoot.MeshId = "http://www.roblox.com/asset/?id=902942089"
...
local titleLabel = tabgodmode:AddLabel("Peace")
titleLabel.TextSize = 20
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local farmKillEnabled = false
local farmKillThread = nil

local farmKillPosition = CFrame.new(-12510.5, -1.6537845134735107, 28019.103515625)

local function startFarmKill()
    if farmKillThread then
        farmKillThread:Disconnect()
        farmKillThread = nil
    end

    farmKillEnabled = true

    farmKillThread = game:GetService("RunService").Heartbeat:Connect(function()
        if not farmKillEnabled then
            farmKillThread:Disconnect()
            return
        end
        ...
        if character then
            ...
            if humanoidRootPart then
                humanoidRootPart.CFrame = farmKillPosition
            end
        end
    end)
end
...
tabgodmode:AddSwitch("Safe Spot", function(state)
    if state then
        startFarmKill()
    else
        stopFarmKill()
...
tabgodmode:AddButton("Auto Safe Spot (Enable after Safe Spot)", function()
    local character = player.Character
    if character then
        local humanoidRootPart = character:FindFirstChild("HumanoidRootPart")
        if humanoidRootPart then
